...
// consists of digits and may contain leading zeros. Every revision contains at least one character.
...
function compareVersion(version1: string, version2: string): number {
    let revisions1: number[] = version1.split(".").map(x => parseInt(x, 10))
    let revisions2: number[] = version2.split(".").map(x => parseInt(x, 10))
    let numRevisions = Math.max(revisions1.length, revisions2.length)

    for (let i = 0; i < numRevisions; i++) {
        let rev1 = (i >= revisions1.length) ? 0 : revisions1[i]
        let rev2 = (i >= revisions2.length) ? 0 : revisions2[i]
...
console.log(compareVersion("7.5.2.4", "7.5.3")) // Output: -1
