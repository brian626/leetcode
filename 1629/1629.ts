...
// had a duration of releaseTimes[i] - releaseTimes[i - 1], and the 0th keypress had a duration of releaseTimes[0].
...
function slowestKey(releaseTimes: number[], keysPressed: string): string {
    const presses: Map<string, number> = new Map<string, number>();

    const keysArray: string[] = keysPressed.split('');

    let overallMaxPress: number = 0;
    for (let i = 0; i < keysArray.length; i++) {
        const maxPress: number = presses.get(keysArray[i]) || 0;
        let pressLength: number = releaseTimes[i];
        if (i > 0) {
            pressLength -= releaseTimes[i - 1];
        }

        presses.set(keysArray[i], Math.max(maxPress, pressLength));
        overallMaxPress = Math.max(overallMaxPress, pressLength);
    }

    const longestPresses: string[] = [];
    for (let [key, value] of presses) {
        if (value === overallMaxPress) {
            longestPresses.push(key);
        }
    }

    return String.fromCharCode(Math.max(...longestPresses.map(x => x.charCodeAt(0))));
};
