...
function getPowerValue(n: number): number {
    let powerValue = 0
    while (n > 1) {
        powerValue++
...
function getKth(lo: number, hi: number, k: number): number {
    let numbersWithPowerValues: [number, number][] = []
    for (let i = lo; i <= hi; i++) {
        numbersWithPowerValues.push([i, getPowerValue(i)])
    }

    numbersWithPowerValues.sort((a, b) => {
        return (a[1] === b[1]) ? a[0] - b[0] : a[1] - b[1]
    })

    return numbersWithPowerValues.map(x => x[0])[k - 1]
};

console.log(getKth(12,15,2)) // 13
console.log(getKth(1, 1, 1)) // Output: 1
console.log(getKth(7, 11, 4)) // Output: 7
// Explanation: The power array corresponding to the interval [7, 8, 9, 10, 11] is [16, 3, 19, 6, 14].
// The interval sorted by power is [8, 10, 11, 7, 9].
// The fourth number in the sorted array is 7.

console.log(getKth(10, 20, 5)) // Output: 13

console.log(getKth(1, 1000, 777)) // Output: 570
