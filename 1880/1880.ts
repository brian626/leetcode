...
function numVal(s: string): number {
    const values: string = s.split('').map(c => String.fromCharCode(c.charCodeAt(0) - 49)).join('');
    return parseInt(values, 10);
}

function isSumEqual(firstWord: string, secondWord: string, targetWord: string): boolean {
    const val1: number = numVal(firstWord);
    const val2: number = numVal(secondWord);
    const val3: number = numVal(targetWord);
