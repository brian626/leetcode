...
function myAtoi(s: string): number {
    let ret = 0
    let sign = 1
    let startedReadingDigits = false
    ...
    for (let i = 0; i < s.length; i++) {
        ...
        if (!startedReadingDigits) {
            if (c === " ") {
                continue
            }
            else if (c === "+") {
                sign = 1
                startedReadingDigits = true
            }
            else if (c === "-") {
                sign = -1
                startedReadingDigits = true
            }
            else if (c.charCodeAt(0) >= zeroChar && c.charCodeAt(0) <= nineChar) {
                ret = (ret * 10) + (c.charCodeAt(0) - zeroChar)
                startedReadingDigits = true
            }
            else {
                break
            ...
            if (c.charCodeAt(0) >= zeroChar && c.charCodeAt(0) <= nineChar) {
                ret = (ret * 10) + (c.charCodeAt(0) - zeroChar)
            }
            else {
                break
...
// console.log(myAtoi("42 and 98"))       // Output: 42
// console.log(myAtoi("--42"))            // Output: 0
// console.log(myAtoi("91283472332"))     // Output: 2147483647
// console.log(myAtoi("+12"))             // Output: 12
// console.log(myAtoi("+-12"))            // Output: 0
