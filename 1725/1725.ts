...
function countGoodRectangles(rectangles: number[][]): number {
    ...
    rectangles.forEach(r => {
        squareLengths.push(Math.min(r[0], r[1]))
    })

    const maxLen = Math.max(...squareLengths)

    return squareLengths.filter(l => (l == maxLen)).length
};

console.log(countGoodRectangles([[5,8],[3,9],[5,12],[16,5]])) // Output: 3
console.log(countGoodRectangles([[2,3],[3,7],[4,3],[3,7]])) // Output: 3
