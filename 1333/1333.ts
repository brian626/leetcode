...
// restaurants with veganFriendlyi set to true) or false (meaning you can include any
// restaurant). In addition, you have the filters maxPrice and maxDistance which are the
...
// lowest. For restaurants with the same rating, order them by id from highest to lowest.
...
function filterRestaurants(restaurants: number[][], veganFriendly: number, maxPrice: number, maxDistance: number): number[] {
    if (veganFriendly == 1) {
        restaurants = restaurants.filter(r => r[2] === 1)
    }

    restaurants = restaurants.filter(r => r[3] <= maxPrice).filter(r => r[4] <= maxDistance).sort((a,b) => {
        if (a[1] === b[1]) {
            return b[0] - a[0]
        }
        else {
...
// Restaurant 1 [id=1, rating=4, veganFriendly=1, price=40, distance=10]
console.log(filterRestaurants([[1,4,0,40,10],[2,8,0,50,5],[3,8,0,30,4],[4,10,0,10,3],[5,1,0,15,1]], 0, 1, 1))
