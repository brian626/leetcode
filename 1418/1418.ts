...
// Given the array orders, which represents the orders that customers have done in a
// restaurant. More specifically orders[i]=[customerNamei,tableNumberi,foodItemi] where
// customerNamei is the name of the customer, tableNumberi is the table customer sit at,
...
function displayTable(orders: string[][]): string[][] {
    let foodItems: Set<string> = new Set<string>()
    let tableFood: Map<string, string[]> = new Map<string, string[]>()

    orders.forEach(order => {
        foodItems.add(order[2])
        let existingFood: string[] = tableFood.get(order[1])
        if (existingFood === undefined) {
            existingFood = []
        }
        existingFood.push(order[2])

        tableFood.set(order[1], existingFood)
    })

    let displayTable: string[][] = []
    displayTable[0] = ["Table"].concat(Array.from(foodItems).sort())

    let tableArray: [string, string[]][] = Array.from(tableFood.entries()).sort((a,b) => {
        return parseInt(a[0], 10) - parseInt(b[0], 10)
    })

    for (let i = 0; i < tableArray.length; i++) {
        const tableNum: string = tableArray[i][0]
        const foodNames: string[] = tableArray[i][1]
        let foodQuantities: string[] = []
        Array.from(foodItems).sort().forEach(food => {
            let numTableFoodItems = foodNames.filter(x => x === food).length
            if (numTableFoodItems > 0) {
                foodQuantities.push(numTableFoodItems.toString())
            }
            else {
                foodQuantities.push("0")
            }
        })

        displayTable[i+1] = [tableNum].concat(foodQuantities)
    }
