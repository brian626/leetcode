...
function copyRandomList(head: Node2 | null): Node2 | null {
    ...
    // First pass: Create the new list
    let copyHead: Node2 = null // new Node2(head.val)
    let sourceNodePtr: Node2 = head
    let copyNodePtr: Node2 = copyHead
    let copyNodeArray: Node2[] = []

    while (sourceNodePtr != null) {
        let copyNode: Node2 = new Node2(sourceNodePtr.val)
        copyNodeArray.push(copyNode)

        if (copyNodePtr == null) {
            copyHead = copyNode
            copyNodePtr = copyNode
        }
        else {
            copyNodePtr.next = copyNode
            copyNodePtr = copyNode
        }
    ...
    // Second pass: Link up all the randoms
    sourceNodePtr = head
    copyNodePtr = copyHead
    while (sourceNodePtr != null) {
        if (sourceNodePtr.random) {
            copyNodePtr.random = copyNodeArray[sourceNodePtr.random.val]
        }
